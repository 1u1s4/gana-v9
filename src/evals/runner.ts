...
async function runCertificationPipelineCheck(config: AgentConfig, runtime: RuntimeContext): Promise<CertificationCheck> {
  ...
  const result = await executeRunPipeline(certificationConfig, {
    ...
    analyzeParlays: async () => ({
      ...
      top: [{
        rank: 1,
        parlayId: 'parlay-ci-certification',
        sourceRunId: runId,
        profile: 'low-odds-top',
        validationStatus: 'won',
        ...
        bankerLegs: [{
          predictionId: 'prediction-ci-certification',
          fixtureId: fixture.id,
          fixture: 'Certification Home vs Certification Away',
          market: 'h2h',
          selection: 'home',
          line: null,
          odds: 1.33,
          confidence: 0.9,
          reason: 'certification banker leg',
        }],
        reasons: ['certification analytical stake recommendation'],
        riskFlags: [],
        legs: [{
          predictionId: 'prediction-ci-certification',
          fixtureId: fixture.id,
