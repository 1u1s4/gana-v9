...
import {
  buildLowOddsPredictionCoverage,
  buildLowOddsScan,
  emptyLowOddsScan,
  fixtureLocalDateKey,
  fixturePriorityIndex,
  lowOddsGlobalDiscoveryConfig,
  mergeFixtureSlates,
  prioritizeFixtureSlate,
  ...
  try {
    lowOddsScan = await runDurableTask('low_odds.scan', 'low-odds-scan.json', async () => {
      ...
      const lowOddsDiscovery = hasProviderDateSlate
        ...
        : await (deps.discoverLowOddsFixtures ?? deps.discoverFixtures ?? discoverFixtures)(lowOddsGlobalDiscoveryConfig(config, LOW_ODDS_GLOBAL_MAX_FIXTURES), {
          date: input.date,
          fullDay: true,
        }, runtime);
