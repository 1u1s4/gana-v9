...
- Fixture labels should include kickoff time in Guatemala when artifact metadata provides it, formatted as `HH:MM` next to the match name. In required-league addendum output, show the kickoff only in the `🌍 Obligatorio` fixture summary lines, not again in required prediction lines or required parlay legs.
...
- Required selected parlay headings should show only the profile emoji and name, for example `💎 principal`, without a status icon or selection count.
...
- Required parlay approaches use the required-league atomic portfolio planner (`principal`, `resultados`, `mixto-seguro`) instead of the daily parlay profile trio; they must use alternate non-blocked fixture projections so the three required coupons are distinct whenever enough candidates exist, while prioritizing confidence, positive atomic edge, positive portfolio edge, market-family diversity, limited leg reuse, and realistic combined odds. Do not force a thematic totals coupon when a mixed positive-edge coupon is stronger.
