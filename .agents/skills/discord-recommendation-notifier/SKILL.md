...
- Required parlay approaches use the required-league atomic portfolio planner (`principal`, `resultados`, `totales`) instead of the daily parlay profile trio; they must use alternate non-blocked fixture projections so the three required coupons are distinct whenever enough candidates exist, while prioritizing confidence, positive atomic edge, market-family diversity, and realistic combined odds.
