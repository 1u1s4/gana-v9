...
- Daily odds-floor strategy snapshot: optional root field `dailyOddsFloorStrategy` in `daily-parlay-recommendations.json`, persisted by Daily E2E after the final recommendation and required-league selection set is known.
...
- When provider-comparison data is available for required-league fixtures, include a separate `📋 Predicciones generales` embed before the required parlays, except for K League 1 where this section is intentionally omitted. Group it by required fixture and show atomic match-analysis markets with the status semaphore (`✅`, `🟡`, `🚫`), a market icon, pick, odds, and integer consensus confidence; do not print market labels like `Resultado:` or `Goles:`, do not append provider names, and do not show edge. Consolidate matching provider selections into one line. For goals, show only the highest-confidence goals pick per fixture, not multiple over/under goal lines.
- When more than one required league is configured, render each league as its own complete addendum (`🌍 Obligatorio`, `📌 Predicciones obligatorias`, optional `📋 Predicciones generales`, and required parlay boxes) and list each required league separately in the header and council summary.
...
- When the artifact contains a valid `dailyOddsFloorStrategy` snapshot with version `odds-floor-highest-confidence-v1`, native multi-message delivery must append one dedicated message after every recommendation/addendum/control payload. Its single embed is titled `🎯 Apuesta analítica del día`. A `selected` snapshot shows the persisted pick legs, published odds, published confidence, the `cuota publicada ≥1.45 · mayor confianza publicada` rule, and an explicit analytical-only/no-execution/no-guarantee note. A `no-eligible-pick` snapshot explicitly says there is no eligible pick. Consume this persisted snapshot exactly as written: the notifier must not reconstruct, rerank, or fall back to the recommendation arrays. Legacy artifacts without the field, and malformed or unsupported snapshots, do not add the strategy message. `--single-message` keeps the strategy as its final embed within the one payload and stays within Discord's 10-embed limit; plain Hermes gateway output adds the equivalent section at the very end.
